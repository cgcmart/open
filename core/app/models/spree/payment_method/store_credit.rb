  ...
  class PaymentMethod::StoreCredit < ::Spree::PaymentMethod
    ...
    def authorize(amount_in_cents, provided_store_credit, gateway_options = {})
      if provided_store_credit.nil?
        ActiveMerchant::Billing::Response.new(false, t('spree.store_credit.unable_to_find'), {}, {})
      else
        action = ->(store_credit) {
    ...
    def handle_action_call(store_credit, action, action_name, auth_code = nil)
      store_credit.with_lock do
        if response = action.call(store_credit)
          ...
          ActiveMerchant::Billing::Response.new(true, t('spree.store_credit_payment_method.successful_action', action: action_name),
    ...
    def handle_action(action, action_name, auth_code)
      ...
      if store_credit.nil?
        ActiveMerchant::Billing::Response.new(false, t('spree.store_credit.unable_to_find_for_action', auth_code: auth_code, action: action_name), {}, {})
      else
