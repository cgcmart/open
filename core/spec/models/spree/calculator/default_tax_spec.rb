...
RSpec.describe Spree::Calculator::DefaultTax, type: :model do
  ...
  let(:starts_at) { nil }
  let(:expires_at) { nil }
  let!(:rate) { create(:tax_rate, tax_category: [tax_category], amount: 0.05, included_in_price: included_in_price, zone: zone, starts_at: starts_at, expires_at: expires_at) }
