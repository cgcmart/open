...
RSpec.describe Spree::Promotion, type: :model do
  ...
  context '#inactive' do
    ...
    it 'is inactive if it has not started yet' do
    ...
    it 'is inactive if it has started already' do
      promotion.starts_at = Time.current - 1.day
      expect(promotion).not_to be_inactive
    end

    it 'is inactive if it has not ended yet' do
      promotion.expires_at = Time.current + 1.day
      expect(promotion).not_to be_inactive
    end
  ...
  context '#active' do
    ...
    it "is not active if it hasn't started yet" do
      promotion.starts_at = Time.current + 1.day
      expect(promotion.active?).to eq(false)
    end

    it 'is not active if it has already ended' do
      promotion.expires_at = Time.current - 1.day
      expect(promotion.active?).to eq(false)
    end
