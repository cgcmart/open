  ...
  module Core
    RSpec.describe Importer::Order do
      ...
      it 'can build an order from API with variant sku' do
        params = { line_items_attributes: { '0' => { sku: sku, quantity: 5 } } }
      ...
      context 'with a different currency' do
        ...
        it 'when a line item price is specified' do
      ...
      context 'variant was soft-deleted' do
        it 'raise error as variant shouldnt be found' do
          ...
          hash = { sku: variant.sku }
          expect { Importer::Order.ensure_variant_id_from_params(hash) }.to raise_error ActiveRecord::RecordNotFound
        end
      end
      ...
      it 'raises with proper message when cant find country' do
        ...
        expect { Importer::Order.ensure_country_id_from_params(address) }.to raise_error ActiveRecord::RecordNotFound
      end
