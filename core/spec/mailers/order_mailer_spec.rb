...
RSpec.describe Spree::OrderMailer, type: :mailer do
  ...
  context 'only shows eligible adjustments in emails' do
    before do
      create(:adjustment, adjustable: order, order: order, eligible: true, label: 'Eligible Adjustment')
      create(:adjustment, adjustable: order, order: order, eligible: false, label: 'Ineligible Adjustment')
    end
  ...
  context 'displays unit costs from line item' do
    ...
    specify do
      ...
      expect(confirmation_email.parts.first.body).not_to include('5.00')
    end
    ...
    specify do
      ...
      expect(cancel_email.parts.first.body).not_to include('5.00')
    end
  end
