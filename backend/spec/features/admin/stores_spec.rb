...
describe 'Stores', type: :feature do
  ...
  context 'when adding a store' do
    ...
    it 'admin is able to set the default_currency' do
      expect(find('#store_default_currency').value).to eq ''

      fill_in 'store_name', with: 'CGCMart Store'
      fill_in 'store_code', with: 'cgcmart'
      fill_in 'store_url', with: 'example.cgcmart.com'
      fill_in 'store_mail_from_address', with: 'from@cgcmart.com'
      select 'CNY', from: 'store_default_currency'
      click_button 'Create'
  ...
  context 'when editing a store' do
    ...
    it 'admin is able to change the default_currency' do
