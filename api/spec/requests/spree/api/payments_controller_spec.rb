...
module Spree
  describe Api::PaymentsController, type: :request do
    render_views
    ...
    context 'as a user' do
      ...
      context 'when the order does not belong to the user' do
        ...
        it 'can view the payments for an order given the order token' do
          get spree.api_order_payments_path(order), params: { order_token: order.token }
          expect(json_response['payments'].first).to have_attributes(attributes)
        end
      end
    end
      ...
      context 'for a given payment' do
        ...
        context 'authorizing' do
          ...
          context 'authorization fails' do
            before do
              ...
              expect_any_instance_of(Spree::Gateway::BogusCreditCard).to receive(:authorize).and_return(fake_response)
              put spree.authorize_api_order_payment_path(order, payment)
            end
        ...
        context 'capturing' do
          ...
          context 'capturing fails' do
            before do
              ...
              expect_any_instance_of(Spree::Gateway::BogusCreditCard).to receive(:capture).and_return(fake_response)
            end
        ...
        context 'purchasing' do
          ...
          context 'purchasing fails' do
            before do
              fake_response = double(success?: false, to_s: 'Insufficient funds')
              expect_any_instance_of(Spree::Gateway::BogusCreditCard).to receive(:purchase).and_return(fake_response)
            end
        ...
        context 'voiding' do
          ...
          context 'voiding fails' do
            before do
              ...
              expect_any_instance_of(Spree::Gateway::BogusCreditCard).to receive(:void).and_return(fake_response)
            end
