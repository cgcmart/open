  ...
  describe Api::CheckoutsController, type: :request do
    ...
    context "PUT 'update'" do
      ...
      it 'transitions a recently created order from cart to address' do
        ...
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token }
      ...
      it 'transitions a recently created order from cart to address with order token in header' do
        expect(order.state).to eq 'cart'
        expect(order.email).not_to be_nil
        put spree.api_checkout_path(order), headers: { 'X-Spree-Order-Token' => order.token }
        expect(order.reload.state).to eq 'address'
      end

      it 'can take line_items_attributes as a parameter' do
        line_item = order.line_items.first
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { line_items_attributes: { 0 => { id: line_item.id, quantity: 1 } } } }
      ...
      it 'can take line_items as a parameter' do
        line_item = order.line_items.first
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { line_items: { 0 => { id: line_item.id, quantity: 1 } } } }
        expect(response.status).to eq(200)
        expect(order.reload.state).to eq 'address'
      end
      ...
      context 'transitioning to delivery' do
        ...
        it 'can update addresses and transition from address to delivery' do
          put spree.api_checkout_path(order),
            params: { order_token: order.token, order: {
      ...
      it 'can update shipping method and transition from delivery to payment' do
        ...
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { shipments_attributes: { "0" => { selected_shipping_rate_id: shipping_rate.id, id: shipment.id } } } }
        expect(response.status).to eq(200)
      ...
      it 'can update the special instructions for an order' do
        ...
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { special_instructions: instructions } }
      ...
      context 'as an admin' do
        ...
        it 'can assign a user to the order' do
          ...
          put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { user_id: user.id, email: "guest@spreecommerce.com" } }
          expect(response.status).to eq(200)
      ...
      it 'can assign an email to the order' do
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { email: "guest@spreecommerce.com" } }
      ...
      it 'can apply a coupon code to an order' do
        ...
        put spree.api_checkout_path(order.to_param), params: { order_token: order.token, order: { coupon_code: "foobar" } }
        expect(response.status).to eq(200)
      end
    ...
    context "PUT 'next'" do
      ...
      it 'cannot transition to address without a line item' do
        ...
        put spree.next_api_checkout_path(order), params: { order_token: order.token }
        expect(response.status).to eq(422)
      ...
      it 'cannot transition if order email is blank' do
        ...
        put spree.next_api_checkout_path(order), params: { id: order.to_param, order_token: order.token }
        expect(response.status).to eq(422)
    ...
    context "PUT 'advance'" do
      ...
      it 'continues to advance an order while it can move forward' do
        ...
        put spree.advance_api_checkout_path(order), params: { order_token: order.token }
      end
